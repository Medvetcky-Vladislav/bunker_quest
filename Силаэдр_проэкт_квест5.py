...
def room_podezd():
    global inventory
    print(f"Вы в комнате подъезд (без газов), вы читаете справочник и находите там, что в этом бункере находятся споры токсичной плесени которые помутняют ваше сознание и распостронняют свои газы, и их газы распространились по комнатам и вокруг бункера из-за неисправной вентиляции")
...
def room_zal():
    ...
    if built["g1"] == True:
        g11 = f"с газами токсичной плесени"
    else:
        g11 = "без газов токсичной плесени"
...
def room_hranilishe():
    ...
    if built["g1"] == True:
        ...
        print(f"Не хотите ли вы починить вентиляцию в этой комнате за {price_food} еды и {price_water} воды (чем больше людей, тем дешевле строительство), чтобы газ токсичной плесени пропал")
        ans6 = input("Введите да или нет ")
        if ans6 == "да":
            print(f"Газа токсичной плесени в этой комнате больше нет")
            built["g1"] = False
        ...
        if built["g1"] == True:
            ...
            if ans7 == "бой":
                ...
                print(f"Через какое-то время вы понимаете, что это не монстры, а другая часть ваших выживших, из-за того что на вас действовал газ {GAS_NAME} вам казалось что это монстры , и вы продолжаете дорогу")
            else:
                print("Ответ введён неверно, введите ответ заново")
                room_hranilishe()

    print(f'Вы в хранилище. Вы пойдёте в комнату зал (без газов {GAS_NAME}) или в комнату генератор, там ничего нет, но есть генератор электричества (без газов {GAS_NAME}) и странная электрическая дверь, или в комнату с запасами воды (без газов {GAS_NAME})')
...
def water_choose_next(g11, g21):
    print(f'Вы в хранилище с водой. Вы пойдёте в комнату хранилище ({g11}) или в комнату родник ({g21}), или в комнату с теплицей (без газов {GAS_NAME})')
...
def room_water():
    ...
    global built
    if loot["l3"] == False:
        print(f"Вы в хранилище воды (без газов {GAS_NAME}), тут 7 воды, вы берёте их")
        loot["l3"] = True
        ...
        if ans8 == "бой":
            ...
            if built["g1"] == True:
                ...
                g11 = f"без газов {GAS_NAME}"
            if built["g2"] == True:
                g21 = f"с газами {GAS_NAME}"
            else:
                g21 = f"без газов {GAS_NAME}"
            if built["wp1"] == False:
                water_repair_pipe(g11, g21)
            else:
                water_choose_next(g11, g21)
...
def room_rodnik():
    ...
    if built["g2"] == True:
            price_food = 7 - inventory["people"]
            price_water = 6 - inventory["people"]
            print(f"Не хотите ли вы починить вентиляцию в этой комнате за {price_food} еды и {price_water} воды (чем больше людей, тем дешевле строительство), чтобы газ {GAS_NAME} пропал")
            ...
            if ans9 == "да":
                ...
                inventory["food"] = inventory["food"] - (7 - inventory["people"])
                pp()
                print(f'Вы в роднике. Вы можете набрать за каждую 1 еду 3 воды или пойти обратно в комнату хранилище с водой (без газов {GAS_NAME})')
                ...
                print(f"Газ {GAS_NAME} в этой комнате остался")
                print("Тут слишком много газа {GAS_NAME}, вы не можете зайти сюда, вы возвращаетесь обратно в хранилище воды")
                room_water()
            else:
                print("Ответ введён неверно, введите ответ заново")
                room_rodnik()
...
def room_teplica():
    global inventory
    print(f'Вы в теплице. Вы можете набрать за каждую 1 воду 2 еды или пойти в комнату хранилище с водой (без газов {GAS_NAME}), или пойти в комнату генератор (без газов {GAS_NAME})')
...
def room_generator():
    ...
    if built["g1"] == True:
        ...
        g11 = f"без газов {GAS_NAME}"
    print(f'Вы в комнате генератор электричества. Вы пойдёте в комнату хранилище ({g11}) или в комнату зал (без газов {GAS_NAME}), или в комнату теплица (без газов {GAS_NAME})')
